...
def show_selector(options, title="请选择一个选项"):
    """使用 msvcrt 创建选择器

    Args:
        options (list): 选项列表 (字符串)
        title (str, optional): 选择器标题. 默认为 "请选择一个选项".

    Returns:
        str or None: 用户选择的选项 (字符串), 如果用户取消则返回 None
    """
    selected_index = 0  # 初始选择索引
    while True:
        os.system('cls' if os.name == 'nt' else 'clear')
        print(f"{COLOR_BOLD}{BG_CYAN} {title} {COLOR_RESET}{COLOR_RESET}\n") # 打印带背景色的标题

        for index, option in enumerate(options): # 遍历选项列表
            if index == selected_index: # 如果是当前选中的选项
                print(f"{BG_GRAY}{COLOR_BOLD}{COLOR_BLACK} > {option} {COLOR_RESET}{COLOR_RESET}") # 高亮显示
            else:
                print(f"   {option}") # 正常显示选项
        print("\n{COLOR_BOLD}操作:{COLOR_RESET} (↑↓ 选择, Enter 确认, ESC/b 返回)") # 操作提示

        key_stroke = msvcrt.getwch() # 获取用户按键

        if key_stroke == ENTER_KEY: # 如果按下 Enter 键
            return options[selected_index] # 返回选中的选项
        elif key_stroke == ESCAPE_KEY or key_stroke.lower() == 'b': # 如果按下 ESC 或 'b' 键
            return None # 返回 None 表示取消
        elif key_stroke == ARROW_UP or key_stroke == '\xe0': # 如果按下上方向键或扩展键引导符
            if key_stroke == '\xe0':
                extended_key = msvcrt.getwch() # 读取扩展键的实际键值
                if extended_key == 'H': # 上箭头
                    selected_index = max(0, selected_index - 1) # 索引减一，向上移动选择
            else:
                selected_index = max(0, selected_index - 1)
        elif key_stroke == ARROW_DOWN or key_stroke == '\xe0': # 如果按下下方向键或扩展键引导符
            if key_stroke == '\xe0':
                extended_key = msvcrt.getwch() # 读取扩展键的实际键值
                if extended_key == 'P': # 下箭头
                    selected_index = min(len(options) - 1, selected_index + 1) # 索引加一，向下移动选择
            else:
                 selected_index = min(len(options) - 1, selected_index + 1) # 索引加一，向下移动选择
        else:
...
def edit_class_plans_menu(files, client_data, subjects_data, time_layouts_data):
    ...
    if not plan_ids:
        print(f"{COLOR_CYAN}没有课表计划可以编辑。{COLOR_RESET}")
        msvcrt.getwch()
        return

    while True:
        if plan_index >= len(plan_ids):
            plan_index = 0
        if plan_index < 0:
            plan_index = len(plan_ids) - 1 if plan_ids else 0
        ...
        if key_stroke == 'e':
            ...
            plan_index = min(len(plan_ids) - 1, plan_index + 1)
        elif key_stroke == '\xe0':
...
def edit_classes_in_plan(files, client_data, subjects_data, time_layouts_data, plan_id):
    ...
    while True:
        ...
        if choice == 'a':
            add_class_to_plan(files, client_data, subjects_data, plan_id)
        elif choice == 'e':
...
def add_class_to_plan(files, client_data, subjects_data, plan_id):
    """添加课程到课表计划"""
    subject_options = [subject['Name'] for subject in client_data['subjects_source']['Subjects'].values()] # 获取科目名称列表
    selected_subject_name = show_selector(subject_options, title="选择要添加的科目") # 使用选择器选择科目

    if selected_subject_name: # 如果用户选择了科目
        subject_id = None
        for s_id, subject in client_data['subjects_source']['Subjects'].items(): # 查找科目 ID
        ...
        if subject_id:
            ...
            client_data['class_plans']['ClassPlans'][plan_id]['Classes'].append(new_class) # 添加课程到课表计划
            save_json(files['class_plans'], client_data['class_plans']) # 保存数据
            print(f"{COLOR_GREEN}课程已添加。{COLOR_RESET}") # 打印成功信息
        else:
            print(f"{COLOR_RED}未能找到科目 ID。{COLOR_RESET}") # 打印错误信息
    else:
        print("添加科目已取消。") # 打印取消信息
...
def edit_class_details(files, client_data, subjects_data, plan_id, selected_class, class_index):
    ...
    while True:
        ...
        if choice == '1':
            subject_options = [subject['Name'] for subject in client_data['subjects_source']['Subjects'].values()] # 科目列表
            selected_subject_name = show_selector(subject_options, title="选择科目") # 使用选择器选择科目
            if selected_subject_name: # 如果选择了科目
                subject_id = None
                for s_id, subject in client_data['subjects_source']['Subjects'].items(): # 查找科目 ID
                    if subject['Name'] == selected_subject_name:
                        subject_id = s_id
                        break
                if subject_id:
                    selected_class['SubjectId'] = subject_id # 更新科目 ID
                    save_json(files['class_plans'], client_data['class_plans']) # 保存
                else:
                    print(f"{COLOR_RED}未能找到科目 ID。{COLOR_RESET}") # 错误信息
        elif choice == '2':
...
def edit_subjects_source_menu(files, client_data):
    ...
    if not subject_ids:
        print(f"{COLOR_CYAN}没有科目可以编辑。{COLOR_RESET}")
        msvcrt.getwch()
        return

    while True:
        if subject_index >= len(subject_ids):
            subject_index = 0
        if subject_index < 0:
            subject_index = len(subject_ids) - 1 if subject_ids else 0
        ...
        if key_stroke == 'e':
            ...
            subject_index = max(0, subject_index - 1)
        elif key_stroke == ARROW_DOWN:
            subject_index = min(len(subject_ids) - 1, subject_index + 1)
        elif key_stroke == '\xe0':
            extended_key = msvcrt.getwch()
            if extended_key == 'H':
                subject_index = max(0, subject_index - 1)
