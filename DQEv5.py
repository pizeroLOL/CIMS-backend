...
# ANSI 转义码用于方向键 -  不再直接使用，改为检测 escape 序列
# ARROW_UP = '\033[A'
# ARROW_DOWN = '\033[B'
# ARROW_LEFT = '\033[D'
# ARROW_RIGHT = '\033[C'
...
def edit_class_plans_menu(files, client_data, subjects_data, time_layouts_data):
    ...
    plan_ids = list(client_data['class_plans']['ClassPlans'].keys()) # Get plan_ids here
    if not plan_ids: # Handle empty plan list case
        print(f"{COLOR_CYAN}没有课表计划可以编辑。{COLOR_RESET}")
        msvcrt.getwch() # Wait for a key press
        return

    while True:
        if plan_index >= len(plan_ids): # Keep index in bounds if plans are deleted
            plan_index = 0
        if plan_index < 0:
            plan_index = len(plan_ids) - 1 if plan_ids else 0 # Handle deletion of first plan
        ...
        if key_stroke == 'e':
            ...
            plan_ids = list(client_data['class_plans']['ClassPlans'].keys()) # Update plan_ids after edit in case of deletion
        elif key_stroke == 'b':
            return
        elif key_stroke == '0':
            exit_program()
        ...
        elif key_stroke == '\xe0': # Arrow keys start with Esc='\xe0' in msvcrt
            extended_key = msvcrt.getwch() # Get the actual arrow key code
            if extended_key == 'H': # Up arrow
                plan_index = max(0, plan_index - 1)
            elif extended_key == 'P': # Down arrow
...
def edit_subjects_source_menu(files, client_data):
    ...
    if not subject_ids: # Handle empty subject list case
        print(f"{COLOR_CYAN}没有科目可以编辑。{COLOR_RESET}")
        msvcrt.getwch() # Wait for key press
        return

    while True:
        if subject_index >= len(subject_ids): # Keep index in bounds if subjects are deleted
            subject_index = 0
        if subject_index < 0:
            subject_index = len(subject_ids) - 1 if subject_ids else 0 # Handle deletion of first subject
        ...
        if key_stroke == 'e':
            ...
            subject_ids = list(client_data['subjects_source']['Subjects'].keys()) # Update subject_ids after edit
        elif key_stroke == 'b':
            return
        elif key_stroke == '0':
            exit_program()
...
def edit_subject_details(files, client_data, subject_id):
    ...
    while True:
        ...
        if choice == '1':
            ...
            new_teacher_name = input("  输入新的科目名称: ")
